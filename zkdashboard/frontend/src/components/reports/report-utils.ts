import { formatEmployeeName, type BasicAttendanceStatus, type IncompleteReason } from '@/lib/api';
import {
  ARGENTINA_TIMEZONE,
...
export function statusLabel(status: BasicAttendanceStatus) {
  switch (status) {
    case 'present':
      return 'Presente';
    case 'incomplete':
...
export function statusClassName(status: BasicAttendanceStatus) {
  switch (status) {
    case 'present':
